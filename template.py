...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "from 0 to 1 and a reasoning string.\n"
            f"Question: {question}\nAnswer: {answer}\nRubric: {rubric}"
        )
        ...
        try:
            ...
            scores = {
                criterion: max(0.0, min(1.0, float(source[criterion])))
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        """
        total = len(results)
        retrieval_recalls = [r.context_recall for r in results if r.context_recall is not None]
        retrieval_precisions = [r.context_precision for r in results if r.context_precision is not None]
        ...
        return {
            ...
            "avg_context_recall": sum(retrieval_recalls) / len(retrieval_recalls) if retrieval_recalls else None,
            "avg_context_precision": sum(retrieval_precisions) / len(retrieval_precisions) if retrieval_precisions else None,
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        def averages(results: list[EvalResult]) -> dict[str, float]:
            count = len(results)
            return {
                "faithfulness": sum(r.faithfulness for r in results) / count if count else 0.0,
                "relevance": sum(r.relevance for r in results) / count if count else 0.0,
                "completeness": sum(r.completeness for r in results) / count if count else 0.0,
            }

        new = averages(new_results)
        baseline = averages(baseline_results)
        regressions = [metric for metric in new if baseline[metric] - new[metric] > 0.05]
        return {
            **{f"new_avg_{metric}": value for metric, value in new.items()},
            **{f"baseline_avg_{metric}": value for metric, value in baseline.items()},
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        """
        scores = {
            "retrieval": failure.faithfulness,
            "relevance": failure.relevance,
            "completeness": failure.completeness,
        }
        lowest = min(scores.values())
        if lowest == failure.faithfulness and failure.faithfulness < 0.5:
            return "Context is missing or irrelevant — improve retrieval"
        if lowest == failure.relevance and failure.relevance < 0.5:
            return "Answer does not address the question — improve prompt clarity"
        if lowest == failure.completeness and failure.completeness < 0.5:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = suggestions[index - 1] if index <= len(suggestions) else "Review failure and add targeted regression test"
            rows.append(
                f"| F{index:03d} | {failure_type} | {self.find_root_cause(failure)} | {suggestion} | Open |"
            )
        return "\n".join(rows)
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0):
            suggestions.append("Implement grounding checks to filter unsupported claims")
        if categories.get("irrelevant", 0) or categories.get("off_topic", 0):
            suggestions.append("Improve intent routing and add focused prompt examples")
        if categories.get("incomplete", 0):
            suggestions.append("Increase retrieval coverage and require all key conditions in answers")
        while len(suggestions) < 3:
            suggestions.append("Add regression cases for recurring failures and monitor metric thresholds")
        return suggestions
